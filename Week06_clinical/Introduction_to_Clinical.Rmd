...
```{r}
write.csv(clinic, "/home1/wfjewell/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/wfjewell/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
# 2. nominal
# 3. 19074 days = 52 years old
# 4. 20211 - 3767 days = 16444 days = 45 years old
```
...
boxplot(
  ...
  ylab = "Centromere 17 Copy Number",
  main = "Cent17 Copy Number by Diagnosis Year",
  col = "lightblue",
  las = 2
)
```
...
```{r}
no_middle_clinic <- clinic$'age_at_diagnosis' > 35 | clinic$'age_at_diagnosis' < 60
```
...
cleaned_clinic$age_category <- ifelse(
  cleaned_clinic$age_at_diagnosis <= 35, "Young",
  ifelse(
    cleaned_clinic$age_at_diagnosis < 50, "Middle",
    "Old"
  )
) 
```
...
Based on the graph - middle, based on deeper analysis - younger (data points become more scarce suggesting treatment/discontinued data)
...
15 years = 5500 days, roughly 30% survival rate
...
p value is greater than 0.05, relationship is not statistically significant.
